...
const EditCar = () => {
    ...
    useEffect(() => {
        const fetchCar = async () => {
            try {
                ...
                if (carData.image) {
                    setImagePreview(`${API_BASE_URL}${carData.image}`);
                }
            } catch (err) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            await axios.put(`${API_BASE_URL}/api/v1/cars/${id}`, carData, {
                headers: {
            ...
            setError(err.response?.data?.message || 'Failed to update car. Please check the details.');
        } finally {
            setLoading(false);
        }
    };
    ...
    return (
        <div className="p-4 sm:p-6 max-w-4xl mx-auto">
            <h1 className="text-2xl font-bold text-white mb-6">Edit Car</h1>
            <form onSubmit={handleSubmit} className="bg-slate-900 p-6 sm:p-8 rounded-xl border border-slate-800 space-y-5">
                ...
                <div>
                    ...
                    {imagePreview && <img src={imagePreview} alt="Preview" className="w-40 h-24 object-cover rounded-md mb-3" />}
                    <input
                        type="file"
                        id="image"
                        name="image"
